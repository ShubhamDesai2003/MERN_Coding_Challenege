    ...
    const fetchData = async () => {
      try {
        const transactionsRes = await axios.get(
          `http://localhost:3000/api/transactions?month=${month}&search=${search}&page=${page}&perPage=${perPage}`
        );
        setTransactions(transactionsRes.data.transactions);
        setTotalTransactions(transactionsRes.data.total);

        const combinedRes = await axios.get(`http://localhost:3000/api/combined?month=${month}`);
